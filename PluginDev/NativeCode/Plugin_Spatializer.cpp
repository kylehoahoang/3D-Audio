...
namespace Spatializer
{
    ...
    class HRTFData
    {
        struct CircleCoeffs
        {
            ...
            void GetHRTF(AudioPluginUtil::UnityComplexNumber* h, float angle, float mix, kiss_fft_cpx* kisshrtf)
            {
                int index1 = 0;
                ...
                for (int n = 0; n < HRTFLEN * 2; n++)
                {
                    ...
                    h[n].im += (hrtf1[1] + (hrtf2[1] - hrtf1[1]) * f - h[n].im) * mix; // Unity
                    kisshrtf[n].r = h[n].re;
                    kisshrtf[n].i = h[n].im; // KissFFT
    ...
    static void GetHRTF(int channel, AudioPluginUtil::UnityComplexNumber* h, float azimuth, float elevation, kiss_fft_cpx* kisshrtf)
    {
        ...
        if (index2 > 12)
            index2 = 12;
        sharedData.hrtfChannel[channel][index1].GetHRTF(h, azimuth, 1.0f, kisshrtf);
        sharedData.hrtfChannel[channel][index2].GetHRTF(h, azimuth, e - f, kisshrtf);
    ...
    UNITY_AUDIODSP_RESULT UNITY_AUDIODSP_CALLBACK ProcessCallback(UnityAudioEffectState* state, float* inbuffer, float* outbuffer, unsigned int length, int inchannels, int outchannels)
    {
        ...
        // KissFFT buffer for HRTF impulse response
        kiss_fft_cpx leftImpulseBuffer[HRTFLEN * 2];
        kiss_fft_cpx rightImpulseBuffer[HRTFLEN * 2];


        GetHRTF(0, data->ch[0].h, azimuth, elevation, leftImpulseBuffer);
        GetHRTF(1, data->ch[1].h, azimuth, elevation, rightImpulseBuffer);


        // Set up KISSFFT
        // Initialize KissFFT configuration for forward and inverse FFT
        kiss_fft_cfg cfg_forward = kiss_fft_alloc(HRTFLEN*2, 0, nullptr, nullptr); // Set forward as 0
        kiss_fft_cfg cfg_inverse = kiss_fft_alloc(HRTFLEN*2, 1, nullptr, nullptr); // Set inverse as 1
        ...
        for (unsigned int sampleOffset = 0; sampleOffset < length; sampleOffset += HRTFLEN)
        {
            for (int c = 0; c < 2; c++)
            {
                ...
                // Create buffers for KISSFFT (Size is HRTFLEN*2 because of left and right audio channels)
                kiss_fft_cpx inputBuffer[HRTFLEN*2];
                kiss_fft_cpx convBuffer[HRTFLEN*2];

                // Places a sample of length HRTFLEN*2 of the sound source into x buffer
                for (int n = 0; n < HRTFLEN * 2; n++)
                {
                    // For Unity SDK
                    ...
                    // For modified KISSFFT
                    inputBuffer[n].r = ch.buffer[n];
                    inputBuffer[n].i = 0.0f;
                }

                // Performs FFT on source buffer after reverb effects and spread are applied to get impulse response
                AudioPluginUtil::FFT::Forward(ch.x, HRTFLEN * 2, false); // Unity SDK
                kiss_fft(cfg_forward, inputBuffer, inputBuffer); // Modified KISSFFT

                // Performs convolution here. Y is the output buffer. X is input buffer (sound). H is hrtf buffer
                for (int n = 0; n < HRTFLEN * 2; n++)
                {
                    AudioPluginUtil::UnityComplexNumber::Mul<float, float, float>(ch.x[n], ch.h[n], ch.y[n]); // Unity SDK

                    if (c == 0) // if left channel
                    {
                        convBuffer[n].r = inputBuffer[n].r * leftImpulseBuffer[n].r - inputBuffer[n].i * leftImpulseBuffer[n].i;
                        convBuffer[n].i = inputBuffer[n].r * leftImpulseBuffer[n].i + inputBuffer[n].i * leftImpulseBuffer[n].r;

                        convBuffer[n].r /= 5.0f;
                        convBuffer[n].i /= 5.0f;
                    }
                    else if (c == 1) // if right channel
                    {
                        convBuffer[n].r = inputBuffer[n].r * rightImpulseBuffer[n].r - inputBuffer[n].i * rightImpulseBuffer[n].i;
                        convBuffer[n].i = inputBuffer[n].r * rightImpulseBuffer[n].i + inputBuffer[n].i * rightImpulseBuffer[n].r;
                ...
                // Inverse FFT with spatialized sound in y
                AudioPluginUtil::FFT::Backward(ch.y, HRTFLEN * 2, false);

                // Process audio for clipping
                //float maxVal = convBuffer[0].r; // Initialize maxVal with the first element of the buffer
                //// Iterate through the buffer to find the maximum value
                //for (int i = 1; i < length; ++i) {
                //    if (convBuffer[i].r > maxVal) {
                //        maxVal = convBuffer[i].r;
                //    }
                //}
                //float threshold = 0.7 * maxVal;
                //float threshold1 = 0.8 * maxVal;
                //for (int n = 0; n < HRTFLEN * 2; n++)
                //{
                //    if (convBuffer[n].r > threshold1) // if left channel
                //    {
                //        convBuffer[n].r *= 0.6f;
                //        convBuffer[n].i *= 0.6f;
                //    }
                //    else if (convBuffer[n].r > threshold) // if left channel
                //    {
                //        convBuffer[n].r *= 0.7f;
                //        convBuffer[n].i *= 0.7f;
                //    }
                ...
                // KISSFFT perform inverse fft on convulted audio
                kiss_fft(cfg_inverse, convBuffer, convBuffer);
                ...
                for (int n = 0; n < HRTFLEN; n++)
                {
                    ...
                    //outbuffer[n * 2 + c] = y;
                    //reverb[n * 2 + c] += y * reverbmix; // Unity SDK

                    float y = s + (convBuffer[n].r * GAINCORRECTION - s) * spatialblend;
                    outbuffer[n * 2 + c] = y;
                    reverb[n * 2 + c] += y * reverbmix; // KISS FFT
                }
            }
